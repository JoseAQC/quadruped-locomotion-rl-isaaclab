...
PKG_ROOT = os.path.join(PROJECT_ROOT, "source", "Antbenchmark")

if PKG_ROOT not in sys.path:
    sys.path.insert(0, PKG_ROOT)
#final error path
import Antbenchmark.tasks  # noqa: F401

# ---------------------------------------------------------------------------
# Fix W&B logging for SKRL 2.x + IsaacLab:
# SKRL writes metrics with its own SummaryWriter, and W&B may only capture
# system metrics. This patch sends SKRL tracking_data directly to wandb.log().
# ---------------------------------------------------------------------------
import numpy as np
...
def _patched_wandb_init(*args, **kwargs):
    # Important: avoid relying on TensorBoard sync
...
def _patched_write_tracking_data_agent(self, *, timestep: int, timesteps: int) -> None:
    wandb_data = {}
    ...
    # Original SKRL behavior: write to TensorBoard and clear tracking data
    _orig_write_tracking_data_agent(self, timestep=timestep, timesteps=timesteps)

    # Extra behavior: send metrics directly to W&B
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    agent_cfg["agent"]["experiment"]["checkpoint_interval"] = 1000

    agent_cfg["agent"]["experiment"]["wandb"] = True
    agent_cfg["agent"]["experiment"]["wandb_kwargs"] = {
        "project": "Locomotion_Ant_benchmark",
    }
